...
AI monitoring and automation system that watches Codex and sends contextual messages when idle.

## Architecture

- **server.js** — Main Node.js server (~2400 lines). Runs on port 3460. Handles:
  - Screenshot capture of the Codex window via `screencapture` + JXA window ID lookup
  ...
  - Message sending via `send_to_codex.py` (AppleScript keystroke typing)
  ...
  - Auto-detects Codex CLI path, Node.js path, memory directory, and session directories

- **brain-worker.js** — Isolated child process for `codex exec --json` calls. Forked by server.js so brain crashes can't take down the server.

- **index.html** — Single-file dashboard UI. Three tabs: Chat, Tracker, CLI. Dark/light mode.

- **send_to_codex.py** — Types messages into Codex via AppleScript.
...
- `findCodex()` / `findSystemNode()` — auto-detect CLI paths at startup
